...
MANIFEST_JSON = json.dumps({
    "name": "Rnai AI WebApp",
    "short_name": "Rnai",
    "description": "Rnai AI Agent & Cowork Platform for Mobile and Desktop Web",
    "start_url": "/",
    "display": "standalone",
    ...
    "icons": [
        {
            "src": "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Crect width='512' height='512' rx='116' fill='%230B3945'/%3E%3Cpath d='M196 196v160' stroke='%23fff' stroke-width='62' stroke-linecap='round'/%3E%3Cpath d='M196 300q0-104 110-104' stroke='%23fff' stroke-width='62' stroke-linecap='round' fill='none'/%3E%3Ccircle cx='382' cy='196' r='34' fill='%23D77757'/%3E%3C/svg%3E",
            "sizes": "512x512",
            "type": "image/svg+xml",
...
<meta name="apple-mobile-web-app-title" content="Rnai">
<link rel="manifest" href="/manifest.json">
<link rel="apple-touch-icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Crect width='512' height='512' rx='116' fill='%230B3945'/%3E%3Cpath d='M196 196v160' stroke='%23fff' stroke-width='62' stroke-linecap='round'/%3E%3Cpath d='M196 300q0-104 110-104' stroke='%23fff' stroke-width='62' stroke-linecap='round' fill='none'/%3E%3Ccircle cx='382' cy='196' r='34' fill='%23D77757'/%3E%3C/svg%3E">
...
input, textarea, select { color:var(--ink); font-weight:600; }
option { color:#171717; }   /* dropdown ที่กางออกใช้พื้นระบบ ต้องมีสีเข้มเสมอ */
::placeholder { color:var(--sub); opacity:0.85; font-weight:400; }
#model .arrow { }
...
.fdcard input { width:100%; border:1.5px solid var(--sub); border-radius:9px; padding:10px 14px;
                font:14px -apple-system,BlinkMacSystemFont,'Inter',sans-serif; color:var(--ink);
                background:var(--page); font-weight:600; transition:.15s; }
.fdcard input:focus { outline:0; border-color:var(--brand-ink); box-shadow:0 0 0 3px rgba(11,57,69,0.15); }
.fdcard input#acEmail, .fdcard input#acPassword { margin-top:8px; font-size:14px; font-weight:600; color:var(--ink); }
...
.setrow input, .setrow select.val { border:1.5px solid var(--sub); border-radius:9px; padding:9px 13px;
             font-size:13.5px; width:260px; font-family:ui-monospace,monospace; color:var(--ink);
             background:var(--page); font-weight:600; letter-spacing:.02em; transition:.15s; }
.setrow input:focus { outline:0; border-color:var(--brand-ink); box-shadow:0 0 0 3px rgba(11,57,69,0.15); }
...
#menubtn { display:none; border:1px solid var(--line); background:var(--card); border-radius:8px; width:34px; height:32px; font-size:16px; color:var(--ink); cursor:pointer; align-items:center; justify-content:center; }
#sideBackdrop { display:none; position:fixed; inset:0; top:57px; background:rgba(0,0,0,0.45); backdrop-filter:blur(2px); z-index:90; transition:opacity .2s; }

@media (max-width:760px) {
  nav { height:52px; padding:0 10px; gap:8px; overflow-x:auto; scrollbar-width:none; }
  nav::-webkit-scrollbar { display:none; }
  #menubtn { display:flex; flex-shrink:0; }
  .brand { font-size:15px; flex-shrink:0; }
  nav a { font-size:12.5px; padding:4px 6px; white-space:nowrap; flex-shrink:0; }
  #acctBtn { padding:4px 8px; font-size:12px; white-space:nowrap; flex-shrink:0; }
  #themebtn { flex-shrink:0; }
  #model { font-size:12px; padding:4px 22px 4px 8px; max-width:125px; flex-shrink:0; }
  
  #wrap { position:relative; }
  #side {
    position:fixed; top:52px; left:0; bottom:0; width:280px; max-width:84vw;
    z-index:95; transform:translateX(-100%); transition:transform .25s ease-in-out;
    box-shadow:4px 0 24px rgba(0,0,0,0.18); border-right:1px solid var(--line); background:var(--card);
  }
  #side.show { transform:translateX(0); }
  #sideBackdrop.show { display:block; }
  
  #main { width:100%; }
  #modebar { padding:8px 10px; flex-wrap:wrap; gap:8px; }
  #modedesc { display:none; }
  #folderchip { max-width:180px; font-size:11.5px; }
  #thread { padding:12px 10px; }
  #composer { padding:10px 10px calc(10px + env(safe-area-inset-bottom)); }
  #box { border-radius:14px; padding:6px 10px; min-height:44px; }
  #input { font-size:14px; }
  #hint { font-size:11px; }
  
  /* Mobile Modals & Touch Friendly Targets */
  .fdcard { width:92vw; max-width:92vw; padding:18px; }
  .fdcard h3 { font-size:16px; }
  .fdcard input { font-size:14px; }
  #attachpop { left:10px; right:10px; width:auto; }
  
  /* Responsive Tables & Code Blocks */
  .bubble table, #thread table { display:block; overflow-x:auto; max-width:100%; }
  .bubble pre, #thread pre { overflow-x:auto; max-width:100%; }
  
  .netcard { width:100%; margin-top:16px; padding:16px; border:1px solid var(--line); border-radius:12px; background:var(--soft); }
}
</style>
</head>
<body>
<nav>
  <button id="menubtn" onclick="toggleMobileSidebar()" title="เมนูประวัติและโปรเจกต์">☰</button>
  <div class="brand">
...
<div id="accountdlg">
  <div class="fdcard" id="acLoginCard">
    <form onsubmit="doLogin(); return false;">
      ...
      <div id="acmsg"></div>
      <div class="actions">
        <button type="button" class="fd-cancel" onclick="$('accountdlg').classList.remove('show')">ยกเลิก</button>
        <button type="submit" class="fd-save" id="acLoginBtn">เข้าสู่ระบบ</button>
      </div>
    </form>
  </div>
...
async function chatDirectCloud(model, text) {
  ...
  // 1. GEMINI
  if (model === 'gemini') {
    if (!geminiKey) throw new Error('กรุณากรอก GEMINI_API_KEY ในหน้า Settings (คลิกปุ่ม 🔑 Login หรือ Settings ด้านบน)');
  ...
  if (model === 'groq') {
    ...
    return { reply, model: 'Groq (Llama-3.3)', elapsed: (performance.now() - t0)/1000 };
  }
  ...
  throw new Error('ไม่พบ API Key ในหน้า Settings\\n\\n• กรุณากดปุ่ม 🔑 Login หรือ Settings ด้านบน เพื่อรับ API Key ฟรีของ Gemini / Groq / OpenRouter แล้วนำมาใส่ในช่อง');
}
